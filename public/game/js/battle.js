...
function Player(name, level, energy, experience, weapon) {
  ...
  this.animateHP = function (damage) {
    playerhp.text = "x" + this.energy;
    if (damage !== 0)
      createjs.Tween.get(fadeToBlack).to({
        alpha: .5
      }, 250).wait(250).to({
        alpha: 0
      }, 250);
...
function Nightmare(name, energy, attackStat, defenceStat, spriteName) {
  ...
  this.initSprite = function (spriteName) {
    ...
    createjs.Tween.get(nightmare.sprite, {
      loop: true
    }).to({
      y: 160
    }, 1000).to({
  ...
  this.animateHP = function (damage) {
    ...
    createjs.Tween.get(nightmareDamageText).to({
      alpha: 1,
      y: nightmareDamageText.y - 20
    }, 1000).call(function () {
    ...
    createjs.Tween.get(hpBarSmall).to({
      scaleX: (this.energy / this.maxEnergy)
    }, 1000).call(function () {
      if (nightmare.isDead)
        createjs.Tween.get(nightmare.sprite).to({
          scaleX: 0,
          scaleY: 0
        }, 750).call(endCombat, [true]);
      else
...
function endCombat(playerWon) {
  ...
  if (playerWon) {
    menuStage.update();
    chestLocked = true;
    createjs.Tween.get(treasureChest).to({
      alpha: 1
    }, 750).call(function () {
      chestLocked = false
    });
